        ...
        for (; i < instruction.operand_count; i++) {
            ...
            if (operand.type == ZYDIS_OPERAND_TYPE_MEMORY &&
                ...
                uint64_t rip = section_virtual_address_ +
                               current_instruction_offset_ + instruction.length;
                uint64_t added =
                    (rip + operand.mem.disp.value >= barrier_ ? 1 : -1) +
                    (rip >= barrier_ ? -1 : 1);
                if (added == 0) {
                    break;
                }
                added = (added > 0 ? 1 : -1) * added_;
...
class Merger {
 ...
 private:
    void handle_lazy_symbol_binding() {
    }
    ...
    void merge_dot_symtab() {
        ...
        uint64_t entry_size =
            output_binary_->get_section(".symtab").entry_size();
                ...
                case ELF_SYMBOL_TYPES::STT_OBJECT:
                    ...
                    symbol.value(
                        ...
                        (output_binary_->sections().begin() + dst_section_id)
                            ->virtual_address() +
                        (dst_binary_->sections().begin() + dst_section_id)
                            ->size());
                    break;
                }
    ...
    void merge_dynamic_symbol_table() {
        ...
        for (; it_current != it_end; it_current++) {
            ...
            if (symbol.shndx() !=
                static_cast<uint16_t>(SYMBOL_SECTION_INDEX::SHN_UNDEF)) {
                symbol.value(
                    output_binary_->get_static_symbol(symbol.name()).value());
            }
            output_binary_->add_dynamic_symbol(symbol);
            added_symbol_num += 1;
        }
        uint64_t entry_size =
            output_binary_->get_section(".dynsym").entry_size();
        assert(entry_size == 24);
    ...
    void merge_dot_rela_dot_plt() {
        ...
        std::for_each(
            ...
            [this, &rela_plt_extend_size, rela_plt_entry_size](
                ...
                re.symbol(
                    &output_binary_->get_dynamic_symbol(reloc.symbol().name()));
                // Add section.
                ...
                const std::string& name =
                    src_binary_->get_section_with_va(reloc.address()).name();
            ...
            [this](const LIEF::ELF::Symbol& imported_symbol) {
                if (imported_symbol.shndx() ==
                    ...
                    const Symbol& exported_symbol =
                        output_binary_->get_static_symbol(
                            imported_symbol.name());
                    if (exported_symbol.shndx() !=
                            static_cast<uint16_t>(
                                SYMBOL_SECTION_INDEX::SHN_UNDEF) &&
                        exported_symbol.binding() ==
                            SYMBOL_BINDINGS::STB_LOCAL) {
                        output_binary_->patch_pltgot(
                            exported_symbol.name(),
                            output_binary_
                                ->get_static_symbol(exported_symbol.name())
                                .value());
                    }
                }
            });
